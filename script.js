...
/** Carrega o estado do localStorage, mesclando com os defaults */
function loadState() {
  try {
...
/** Salva STATE no localStorage e mostra feedback */
function saveChanges() {
  const ok = saveState(STATE);
  showSaveFeedback(ok ? '✓ Alterações salvas com sucesso!' : '✗ Erro ao salvar.');
...
function importConfig(e) {
  ...
  reader.onload = (ev) => {
    try {
      ...
      loadChangesIntoPanel();
      navigateTo(currentPage);
      setBackupStatus('✓ Configurações importadas com sucesso!');
    } catch {
...
function resetSite() {
  const confirmed = window.confirm('Tem certeza? Todas as alterações serão perdidas.');
  ...
  loadChangesIntoPanel();
  navigateTo(currentPage);
  setBackupStatus('✓ Site resetado para o padrão.');
}
...
function init() {
  // 1. Renderiza a página inicial
  navigateTo('inicio');

  // 2. Registra cliques na navbar principal
  document.querySelectorAll('.nav__link').forEach(link => {
  ...
  // 6. Aplica textos persistidos em elementos estáticos (logo, footer, whatsapp)
  applyStaticEditable();
}
